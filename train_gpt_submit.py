"""
train_gpt_submit.py — Taking techniques from https://github.com/openai/parameter-golf/pull/315
and adding in improved compression
"""
...
# Instead, we get approximately the same model (with a small hit) by quantizing to int6/int8 and compressing.
...
# -----------------------------
# OPTIMIZED SERIALIZATION (LZMA sparse encoding)
# Separate streams + transpose + type-group interleave + sparse decomposition.
# ~11% smaller than torch.save + zstd-22 on typical quantized models.
# -----------------------------

def _encode_lzma_sparse(quant_result: dict[str, Tensor], quant_meta: dict[str, object]) -> bytes:
    """Encode quantized model with LZMA sparse compression.

    Replaces torch.save + zstd-22 with:
    - Separate streams per data type (int6 weights, int8 weights, scales, passthrough)
    - Transpose 2D tensors (column patterns compress better)
    - Group same tensor types across layers with row-interleaving
    - Sparse decomposition for int6 weights (bitmask + signs + abs==1 mask + abs>1 values)
    - Combined masks stream (zero-mask + abs-mask) for better cross-mask patterns
    - Combined misc stream (int8 + scales + byte-shuffled passthrough)
    - LZMA RAW format (no XZ container overhead)
    """
    w = quant_result
    m = quant_meta
    preset = 9 | lzma.PRESET_EXTREME

    type_groups: dict[str, list[tuple[str, np.ndarray]]] = {}
    ...
    # Compress streams (combine masks + combine misc for fewer streams)
    ...
    # Combine int8 + scales + byte-shuffled passthrough into one misc stream
...
def _decode_lzma_sparse(blob: bytes) -> tuple[dict[str, Tensor], dict[str, object]]:
    """Decode LZMA sparse format back to (quant_result, quant_meta)."""
    buf = io.BytesIO(blob)
    ...
    # Read 4 length-prefixed compressed streams
    blobs = []
    for _ in range(4):
    ...
    # Reconstruct dense int6 weight array from sparse decomposition
    ...
    # Reconstruct tensors from manifest
    w: dict[str, Tensor] = {}
    s_offset = 0
    p_offset = 0
    q8_offset = 0
    ...
    q_buf_io = io.BytesIO(q6_dense.tobytes())
    for suffix in sorted(type_groups.keys()):
        group = type_groups[suffix]
        ...
        if len(shapes) == 1 and len(group[0][1]) == 2:
            ...
            interleaved = np.frombuffer(q_buf_io.read(total_bytes), dtype=np.int8)
            ...
            for name, shape in group:
                ...
                for s in shape:
                    nbytes *= s
                raw_bytes = q_buf_io.read(nbytes)
                if len(shape) == 2:
...
def main() -> None:
    ...
    quant_blob = _encode_lzma_sparse(quant_result, quant_meta)
    if master_process:
        with open("final_model.int6.ptz", "wb") as f:
            f.write(quant_blob)
        quant_file_bytes = len(quant_blob)
        code_bytes = len(code.encode("utf-8"))
        log0(f"Serialized model int6+lzma_sparse: {quant_file_bytes} bytes")
        log0(f"Total submission size int6+lzma_sparse: {quant_file_bytes + code_bytes} bytes")
    ...
    quant_w, quant_m = _decode_lzma_sparse(quant_blob_disk)
