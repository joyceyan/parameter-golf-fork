...
2. **Design the next experiment** based on insights from the notebook. Check the "Ideas queue" section of `notes.md` first for queued ideas, then consult the "Record analysis" section for implementation-ready descriptions of record-setting techniques (exact hyperparameters, code changes, formulas). **Prioritize reproducing proven record techniques before inventing new approaches.** Each record entry has enough detail to port the technique to `train_gpt_mlx.py` without reading the original submission.
